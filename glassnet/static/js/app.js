...
function renderEvidence(value) {
  const evidence = String(value || "No evidence was recorded.");
  if (evidence.length <= 180) return `<strong class="mono evidence-short">${escapeHtml(evidence)}</strong>`;

  const preview = `${evidence.slice(0, 180).trim()}…`;
  return `<div class="evidence-block"><code class="evidence-preview">${escapeHtml(preview)}</code><details class="evidence-details"><summary>View full evidence</summary><code>${escapeHtml(evidence)}</code></details></div>`;
}
