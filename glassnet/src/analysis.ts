...
function buildSecurityChecks(report: AnalysisInput): SecurityCheck[] {
  ...
  for (const [key, name, purpose] of headers) {
    ...
    checks.push(check({ id: `HEADER_${key.toUpperCase().replaceAll("-", "_")}`, name, status: value ? "Passed" : "Needs review", severity: "low", confidence: "high", observation: value ? `${name} was present.` : `${name} was not present in the observed main response.`, why_it_matters: purpose, evidence: value || "Header absent from the captured response.", limitation: "A missing header alone does not prove that the website is insecure.", beginner_explanation: value ? `The page supplied this browser protection.` : `GlassNet did not see this optional browser protection on the scanned response.` }));
  }

  checks.push(
    check({ id: "COOKIE_SECURITY", name: "Cookie security", status: cookieWarnings.length ? "Concern" : "Passed", severity: cookieWarnings.length ? "medium" : "info", confidence: "medium", observation: cookieWarnings.length ? `${cookieWarnings.length} contextual cookie warning(s).` : report.cookies.length ? "No contextual cookie-attribute warning was produced." : "No cookies were created during this scan.", why_it_matters: "Secure, HttpOnly, and SameSite attributes can reduce specific cookie risks.", evidence: cookieWarnings.join("; ") || (report.cookies.length ? "Captured cookie attributes." : "The cookie collection was empty."), limitation: "Cookie purpose can be uncertain, and some cookies intentionally need JavaScript access.", beginner_explanation: cookieWarnings.length ? "One or more cookies may be missing a protection relevant to their likely purpose." : report.cookies.length ? "No major cookie-attribute concern was identified from the available metadata." : "There were no observed cookies whose attributes needed checking." }),
